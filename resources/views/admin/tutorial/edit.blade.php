...
@section('title', 'Edit Tutorial - Admin CIO')
@section('page_title', 'Edit Tutorial')
...
<style>
    ...
    .thumbnail-preview { max-width: 200px; max-height: 120px; object-fit: cover; border-radius: 8px; margin-top: 8px; }
...
@section('content')
    <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
            <form method="POST" action="{{ url('/cms/tutorial/' . $tutorial->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-section">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="judul" class="form-control" value="{{ $tutorial->judul }}" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Kategori</label>
                            <input type="text" name="kategori" class="form-control" value="{{ $tutorial->kategori }}" required>
                        </div>
                    </div>
                    <div class="mt-4">
                        <label class="form-label">Deskripsi (Singkat)</label>
                        <textarea name="deskripsi" class="form-control" rows="3">{{ $tutorial->deskripsi }}</textarea>
                    </div>
                    <div class="row g-4 mt-2">
                        <div class="col-md-6">
                            <label class="form-label">Penulis</label>
                            <input type="text" name="penulis" class="form-control" value="{{ $tutorial->penulis }}" placeholder="Nama penulis">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Urutan</label>
                            <input type="number" name="urutan" class="form-control" value="{{ $tutorial->urutan }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label class="form-label">Upload Thumbnail Baru (jpeg, png, jpg, webp, max 2MB)</label>
                            <input type="file" name="thumbnail" class="form-control" accept="image/jpeg,image/png,image/jpg,image/webp" id="thumbnailInput">
                            @if($tutorial->thumbnail)
                                <img class="thumbnail-preview mt-2" src="{{ asset('storage/' . $tutorial->thumbnail) }}" alt="Current thumbnail" id="thumbnailPreview">
                            @else
                                <img class="thumbnail-preview" id="thumbnailPreview" alt="Preview" style="display:none;">
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="form-section">
                    ...
                    <div class="form-section-divider"></div>
                    <textarea name="konten" id="editor" rows="10">{{ $tutorial->konten }}</textarea>
                </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <select name="tags[]" class="form-select tags-select" multiple>
                                @foreach($tags as $tag)
                                    <option value="{{ $tag->id }}" {{ $tutorial->tags->contains($tag->id) ? 'selected' : '' }}>{{ $tag->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <div class="form-check form-switch">
                                <input type="checkbox" name="is_featured" class="form-check-input" id="isFeatured" value="1" {{ $tutorial->is_featured ? 'checked' : '' }}>
                                <label class="form-check-label" for="isFeatured">Tampil di Halaman Home</label>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-check form-switch">
                                <input type="checkbox" name="is_active" class="form-check-input" id="isActive" value="1" {{ $tutorial->is_active ? 'checked' : '' }}>
                ...
                <div class="mt-5 pt-3 border-top d-flex justify-content-end gap-2">
                    ...
                    <button type="submit" class="btn btn-primary px-4 shadow-sm">
                        <i class="fa-solid fa-save me-1"></i> Update Tutorial
                    </button>
                </div>
            </form>
        </div>
